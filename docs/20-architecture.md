...
| `RendererPort` | Present a derived **`TerrainFrame`** snapshot — grid dims, row-major heights, (grown per [ADR 0023](./decisions/0023-derived-terrain-snapshot-per-vertex-type.md)) row-major per-vertex terrain **types**, and (Phase 2) the **waterline** datum the renderer floats its living water surface at; all *derived* read-only data, no simulation or camera/view state ([ADR 0020](./decisions/0020-workbench-runtime-and-rendererport.md)). A frame built only to read heights (picking) carries an empty `types` slice and an unread waterline. The camera is adapter-local view state and never crosses the boundary. | on-screen renderer (`wgpu`/Metal, [ADR 0007](./decisions/0007-wgpu-rendering-framework.md)); headless render-to-PNG (agent visual self-check); no-op test double. |
| `SimDriver` | The interactive seam ([ADR 0022](./decisions/0022-interactive-shaping-seam-input-command-simdriver.md)): the renderer *holds* one to `submit` a discrete **`TerrainCommand`** (`Raise`/`Lower` at integer grid coords) and pull the current snapshot (`width`/`height`/`heights`/`types`/`revision`) to draw — `types` being the interactive twin of the `TerrainFrame` type channel ([ADR 0023](./decisions/0023-derived-terrain-snapshot-per-vertex-type.md)), re-derived whenever an edit moves the field. The waterline is the session-constant datum, cached from the initial `present` rather than re-pulled, so the water surface tracks live edits (the coastline moves as terrain crosses the fixed datum) with no new pull. Sits alongside `RendererPort`, which is unchanged. | app `WorkbenchSession` (owns the core `World` + recorded command log); the static renderer adapters do not implement it. |

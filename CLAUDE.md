...
cargo run -p providence -- capture-shape [BEFORE MID AFTER]   # display-free proof: submit a command, capture the animation tween at t=0/0.5/1
...
| `providence-renderer` | [`adapters/renderer`](adapters/renderer) | **Workbench renderer adapter** ([ADR 0020](docs/decisions/0020-workbench-runtime-and-rendererport.md)): realises `RendererPort`, drawing a derived `TerrainFrame` as a lit 3D surface (on-screen, headless-PNG, and no-op adapters); the camera is adapter-local view state. Now also **drives the interactive shaping seam** ([ADR 0022](docs/decisions/0022-interactive-shaping-seam-input-command-simdriver.md); issue #9 Phase 2/3): the on-screen window *holds* `&mut dyn SimDriver`, maps a cursor-picked click to a `TerrainCommand` per the `input.shape.*` bindings, submits it, then pulls the fresh snapshot and **eases** the drawn surface old→new via a render-only mesh tween (`anim`, `render.animation.duration_ms`) — importing only the `SimDriver` trait, never the core. `wgpu`/`winit` are confined here; the pure input mapping (cursor→command, click-vs-drag) and tween maths are gate-tested. |

...
- **`render.*`** — presentation for the workbench renderer, projected into a standalone `RenderParams` outside the determinism boundary ([ADR 0020](./decisions/0020-workbench-runtime-and-rendererport.md)): `camera` (initial pose + projection lens; plus the orbit/pan/zoom controller's bounds `min_distance`/`max_distance`/`min_pitch_degrees`/`max_pitch_degrees` and sensitivities `orbit_speed`/`pan_speed`/`zoom_speed` — issue #8 Phase 2), `lighting`, `palette`, `background`, `mesh` (`vertical_scale`), `window` (`width`/`height`, also the headless-capture size) — issue #8 — and `hud` (the read-only debug overlay's `enabled`/`show_camera`/`show_reticle` toggles — [ADR 0015](./decisions/0015-debug-hud-ui-layer.md); issue #8 Phase 3; drawn only under the renderer's `debug-hud` feature). The cascade-**animation** timing `render.animation.duration_ms` (issue #9/#10 Phase 3, [ADR 0022](./decisions/0022-interactive-shaping-seam-input-command-simdriver.md) §5) — how long the drawn surface eases from its old shape to a shaping command's new one; render-only, so no wall-clock or float reaches the core (`0` snaps instantly).
